...
独立的、只聚焦自达号的业绩看板。日流水与原蕉下看板**共享数据源**（`6月业绩追击（纯直播）.xlsx`），花名册保留独立的 `6月总自达号业绩.xlsx` Sheet2。更新一次 Excel 两个看板同步刷新。
...
| 数据用途 | 来源 Excel | Sheet |
|---------|-----------|-------|
| **花名册**（达人归属） | `CC/自达号业绩看板/6月总自达号业绩.xlsx` | `Sheet2`（col 1 昵称、col 2 抖音号、col 3 机构归属） |
| **日流水**（GMV/消耗等） | `Desktop/.../6月业绩追击（纯直播）.xlsx`（与原看板共享） | `X月直播数据`（自动查找） |

> 日流水列映射：col 3 抖音号、col 4 日期、col 6 时长、col 26 GMV、col 27 支付、col 32 退款、col 34 佣金、col 44 投放绑定、col 45 投放被投
